...
def filter_dataset(pd_data):
    ...
    tokenizer = CLIPTokenizer.from_pretrained(
        "CompVis/stable-diffusion-v1-4", subfolder="tokenizer", cache_dir=".cache"
    )
    ignore = []
...
class PNGImageDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.data = (
            prompts_df[["prompt", "evaluation_seed", "evaluation_guidance"]]
            if "evaluation_seed" in prompts_df.columns
            else prompts_df[["prompt"]]
        )
        ...
        self.image_files.extend(
            [(f, subdir) for f in os.listdir(dir_path) if f.endswith(".png")]
        )
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_path).convert(
            "RGB"
        )  # Reads image and returns a CHW tensor
        # image = TF.to_tensor(image)
        prompt = self.data.iloc[idx].prompt
        seed = (
            self.data.iloc[idx].evaluation_seed
            if "evaluation_seed" in self.data.columns
            else None
        )
        guidance_scale = (
            self.data.iloc[idx].evaluation_guidance
            if "evaluation_guidance" in self.data.columns
            else 7.5
        )
        if self.transform:
...
class EvalImageDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, concept, class_name, transform=None):
        self.root_dir = root_dir
        self.transform = transform
        self.image_files = []
        self.prompts = []
        ...
        for f in glob(os.path.join(root_dir, self.concept, self.class_name, "*.jpg")):
            self.prompts.append(
                f"An image of {self.class_name} in {self.concept.replace('_', ' ')} style."
            )
    ...
    def __getitem__(self, idx):
        ...
        return image, prompt, None, guidance_scale

def get(root_dir, concept, class_name):

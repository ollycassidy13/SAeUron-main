...
class TextGrad(Attacker):
    ...
    def run(self, task, logger):
        image, prompt, seed, guidance = task.dataset[
            min(self.attack_idx, len(task.dataset) - 1)
        ]
        ...
        if not self.universal:
            ...
            if self.sequential:
                for t in task.sampled_t:
                    ...
                    id_list = [
                        id for id in id_list if id != task.tokenizer.eos_token_id
                    ]
                    ...
                    if results.get("success") is not None and results["success"]:
                        logger.save_img("success", results.pop("image"))
                    else:
                        results.pop("image")

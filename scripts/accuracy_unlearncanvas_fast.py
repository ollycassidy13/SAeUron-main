...
def main(
    ...
    input_dir = os.path.join(input_dir, theme) if theme is not None else input_dir
    ...
    for batch_images, batch_style_labels in tqdm(style_dataloader):
        ...
        if not dry_run:
            style_output_path = os.path.join(output_dir, f"{theme}.pth")

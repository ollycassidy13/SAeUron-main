...
def main(
    ...
    multiplier=-1.0,
    guidance_scale=9.0,
    output_dir="eval_results/mu_results/style_sequential/",
):
    ...
    progress_bar = tqdm(
        sequential_themes_to_unlearn,
        total=len(sequential_themes_to_unlearn),
        disable=not accelerator.is_main_process,
    )
    for themes_to_unlearn in progress_bar:
        if accelerator.is_main_process:
            progress_bar.set_description(f"Unlearning {themes_to_unlearn}")
        output_path = os.path.join(
            output_dir,
            f"percentile_{percentile}_multiplier_{multiplier}/{'_'.join(themes_to_unlearn)}",
        )
        ...
        for test_theme in theme_avail:
            ...
            with accelerator.split_between_processes(
                ...
                steering_hooks[hookpoint] = hooks.SAEMaskedUnlearningHook(
                    concept_to_unlearn=themes_to_unlearn,
